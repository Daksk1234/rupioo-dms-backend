...
import { ProductSection } from "../model/productSection.model.js";
...
export const saveProductSection = async (req, res) => {
  try {
    const user = await User.findById({ _id: req.body.created_by });
    if (!user) {
      return res.status(400).json({ message: "User Not Found", status: false });
    ...
    req.body.database = user.database;
    const existingProductSection = await ProductSection.findOne({
      name: req.body.name,
      database: req.body.database,
      status: "Active",
    });
    ...
    const productSection = await ProductSection.create(req.body);
...
export const ViewProductSection = async (req, res, next) => {
  try {
    const userId = req.params.id;
    // const adminDetail = await getCategoryHierarchy(userId);
    // const adminDetail = adminDetails.length === 1 ? adminDetails[0] : adminDetails;
    ...
    //     return res.status(404).json({ error: "Category Not Found", status: false })
    // }
    let productSections = await ProductSection.find({
      database: database,
      status: "Active",
    }).sort({ sortorder: -1 });
    return productSections
      ? res.status(200).json({ ProductSection: productSections, status: true })
      : res.status(404).json({ error: "Not Found", status: false });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: err, status: false });
...
export const ViewProductSectionById = async (req, res, next) => {
  try {
    let productSections = await ProductSection.findById({
      _id: req.params.id,
    }).sort({
      sortorder: -1,
    });
...
export const DeleteProductSection = async (req, res, next) => {
  try {
    const productSections = await ProductSection.findById({
      _id: req.params.id,
    });
    if (!productSections) {
      return res.status(404).json({ error: "Not Found", status: false });
    }
    productSections.status = "Deactive";
    await productSections.save();
    return res.status(200).json({ message: "delete successful", status: true });
  } catch (err) {
    console.log(err);
    return res.status(500).json({ error: err, status: false });
...
export const UpdateProductSection = async (req, res, next) => {
  try {
    const productSectionId = req.params.id;
    const existingProductSection = await ProductSection.findById(
      productSectionId
    );
    if (!existingProductSection) {
      return res
      ...
      const updatedProductSection = req.body;
      await ProductSection.findByIdAndUpdate(
        productSectionId,
        updatedProductSection,
        {
          new: true,
        }
      );
...
export const updateSteps = async (req, res) => {
  try {
    ...
    if (!substep) {
      return res
        .status(404)
        .json({ message: "SubStep not found", status: false });
    }
    substep.step_Name = step_Name || substep.step_Name;
    substep.note = note || substep.note;
    const updatedCategory = await category.save();
    return res.status(200).json({
