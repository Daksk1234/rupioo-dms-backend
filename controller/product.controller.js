...
export const DeleteProduct = async (req, res, next) => {
  try {
    ...
    if (purchageOrder && purchageOrder.length > 0) {
      return res.json({
        message: "Product is used in Purchase order",
        status: false,
      });
    }
    if (salesOrder && salesOrder.length > 0) {
      return res.json({
        message: "Product is used in Sales order",
        status: false,
      });
    }
...
export const StockAlert = async (req, res) => {
  try {
    ...
    const product = await Product.find({
      ...
      .populate({ path: "partyId.partyId", model: "customer" })
      .populate({ path: "warehouse", model: "warehouse" });
    if (product.length === 0) {
      return res
        .status(404)
    ...
    product.forEach((item) => {
      if (item.qty < item.MIN_stockalert) {
        let StockAlerts = {
          ...
          SupplierName:
            item?.partyId[item.partyId.length - 1]?.partyId.ownerName || null,
          SupplierGST:
            item?.partyId[item.partyId.length - 1]?.partyId.gstNumber || null,
...
export const addProductInStock = async (warehouse, warehouseId, id) => {
  try {
    const user = await Stock.findOne({
      ...
      "productItems.productId": id._id.toString(),
    });
...
export const addProductInWarehouse4 = async (
  warehouse,
  warehouseId,
  orderItem,
  date
  ...
  try {
    ...
    if (!stock) {
      ...
      if (stock.length === 0) {
        ...
        for (let item of stock) {
          ...
          if (existingStock) {
            if (item.date.toDateString() === dates.toDateString()) {
              ...
              existingStock.totalPrice += orderItem.qty * orderItem.price;
              existingStock.pRate =
                existingStock.totalPrice / existingStock.pQty || 0;
              item.markModified("productItems");
              await item.save();
            } else {
              existingStock.gstPercentage = warehouse.GSTRate;
        ...
        const existProductInStock = await Stock.findOne({
          warehouseId: warehouseId.toString(),
          date: { $gte: startOfDay, $lte: endOfDay },
        });
...
export const addProductInWarehouse5 = async (
  warehouse,
  warehouseId,
  orderItem,
  date
        ...
        if (existProductInStock) {
          ...
          if (!existingProduct) {
            let productItems = {
              ...
              price: warehouse.Purchase_Rate,
              totalPrice:
                (Number(warehouse.qty) || 0) * (Number(orderItem.price) || 0),
              oQty: warehouse.Opening_Stock,
              oRate: warehouse.Purchase_Rate,
              oTaxRate: warehouse.GSTRate,
              oTotal: warehouse.qty * warehouse.Purchase_Rate,
...
export const addProductInWarehouse6 = async (
  warehouse,
  warehouseId,
  orderItem,
  date
        ...
        for (let item of stock) {
          ...
          if (existingStock) {
            if (item.date.toDateString() === dates.toDateString()) {
              ...
              existingStock.pendingStockTotal +=
...
export const addProductInWarehouse7 = async (
  warehouse,
  warehouseId,
  orderItem,
  date,
...
export const addProductInWarehouse8 = async (
  warehouse,
  warehouseId,
  orderItem,
  date
  ...
  try {
    ...
    if (stock) {
      ...
      if (existingStock) {
        existingStock.pendingStock -= orderItem.qty;
        existingStock.pendingStockTotal -=
          orderItem.qty * warehouse.Purchase_Rate;
        stock.markModified("productItems");
        await stock.save();
      }
    }
...
export const addProductInWarehouse9 = async (
  warehouse,
  warehouseId,
  orderItem,
  date1
    ...
    if (!stock) {
      ...
      if (stock.length === 0) {
        ...
        for (let item of stock) {
          ...
          // return qty
          const stock1 = await Stock.findOne({
            warehouseId: warehouseId.toString(),
...
const getProfitColorFromPercentage = (profitPercentage, colorConfig = {}) => {
  ...
  // If config is incomplete, just bail out
  if (!green || !orange || !blue || !red) return null;

  if (p >= 15) return green; // ≥ 15%
  if (p >= 10) return orange; // 10 – 14.99%
  if (p >= 5) return blue; // 5 – 9.99%
  ...
  try {
    ...
    if (!Array.isArray(productColors) || productColors.length === 0) {
      return res.status(400).json({
        status: false,
        message: "No productColors provided",
      });
    }
