...
export const saveOtherCharges = async (req, res, next) => {
  try {
    ...
    return otherCharges
      ? res
          .status(200)
          .json({ message: "data saved successfull", status: true })
      : res
          .status(400)
...
export const viewOtherCharges = async (req, res, next) => {
  try {
    const otherCharges = await OtherCharges.find({
      database: req.params.database,
      status: "Active",
    }).sort({ sortorder: -1 });
    return otherCharges.length > 0
      ? res.status(200).json({ OtherCharges: otherCharges, status: true })
...
export const updatedOtherCharges = async (req, res, next) => {
  try {
    ...
    await OtherCharges.findByIdAndUpdate(req.params.id, updatedData, {
      new: true,
    });
    return res
      .status(200)
