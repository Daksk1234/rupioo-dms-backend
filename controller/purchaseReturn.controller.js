...
export const viewPurchaseReturn = async (req, res, next) => {
  try {
    ...
    // }
    const purchaseReturn = await PurchaseReturn.find({ database: database })
      .sort({ sortorder: -1 })
      .populate({ path: "returnItems.productId", model: "product" })
      .populate({ path: "orderId", model: "purchaseOrder" })
      .populate({ path: "userId", model: "user" })
      .populate({ path: "partyId", model: "customer" });
    return purchaseReturn
      ? res.status(200).json({ PurchaseReturn: purchaseReturn, status: true })
...
export const viewPurchaseReturnByUserId = async (req, res, next) => {
  try {
    const purchaseReturn = await PurchaseReturn.find({ userId: req.params.id })
      .sort({ sortorder: -1 })
      .populate({ path: "returnItems.productId", model: "product" })
      .populate({ path: "orderId", model: "purchaseOrder" })
      .populate({ path: "userId", model: "user" })
  ...
  try {
    ...
    if (!existingPurchaseReturn) {
      return res
        .status(404)
        .json({
          message: `Sales return with ID ${purchaseReturnId} not found`,
          status: false,
        });
    }
    ...
    const promises = updateData.productItems.map(
      async ({ productId, qtyReturn }) => {
    ...
    const updatedSalesReturn = await PurchaseReturn.findOne({
      _id: purchaseReturnId,
    });
    return res
      .status(200)
...
export const savePurchaseReturnOrder = async (req, res) => {
  ...
  try {
    ...
    const promises = returnItems.map(
      async ({ productId, qtyReturn, qtyPurchase, price }) => {
        const product = await Product.findOne({ _id: productId });
        const order = await PurchaseOrder.findOne({ _id: orderId });
        if (!order) {
          throw new Error(`Order ${orderId} not found`);
    ...
    return res
      .status(200)
      .json({
        message: "Purchase Order returns processed successfully",
        orderReturns,
      });
  } catch (error) {
    console.error(error);
