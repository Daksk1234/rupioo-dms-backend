...
export const SaveTransporter = async (req, res) => {
  try {
    if (req.body.id) {
      ...
      return res
        .status(400)
        .json({ message: "transporter id required", status: false });
    }
    if (req.file) {
    ...
    return transporter
      ? res
          .status(200)
          .json({ message: "transporter save successfully", status: true })
      : res
          .status(400)
...
export const ViewTransporter = async (req, res, next) => {
  try {
    ...
    let transporter = await Transporter.find({
      database: database,
      status: "Active",
    })
      ...
      .sort({ sortorder: -1 });
    return transporter
      ? res.status(200).json({ Transporter: transporter, status: true })
...
export const ViewTransposrterById = async (req, res, next) => {
  try {
    let transporter = await Transporter.findById({
      _id: req.params.id,
      status: "Active",
    }).populate({ path: "rolename", model: "role" });
    return transporter
...
export const UpdateTransporter = async (req, res, next) => {
  try {
    ...
    if (!existingTransporter) {
      return res
        .status(404)
        .json({ error: "transporter not found", status: false });
    } else {
      if (req.body.serviceArea) {
        req.body.serviceArea = JSON.parse(req.body.serviceArea);
      }
      ...
      await Transporter.findByIdAndUpdate(transporterId, updatedTransporter, {
        new: true,
      });
      return res
        .status(200)
        .json({ message: "Transporter Updated Successfully", status: true });
    }
  } catch (err) {
    console.error(err);
    return res
    ...
    for (let rowIndex = 2; rowIndex <= worksheet.actualRowCount; rowIndex++) {
      ...
      if (document.database) {
        ...
        if (!role) {
          ...
          if (document.id) {
            const existingId = await Transporter.findOne({
              id: document.id,
              database: document.database,
              status: "Active",
            });
  ...
  try {
    ...
    for (let rowIndex = 2; rowIndex <= worksheet.actualRowCount; rowIndex++) {
      ...
      if (!role) {
        ...
        const existTransporter = await Transporter.findOne({
          id: document.id,
          database: document.database,
          status: "Active",
        });
        if (!existTransporter) {
          ...
          const insertedDocument = await Transporter.findOneAndUpdate(
            filter,
            document,
            options,
          );
