...
export const generateOrderNo = async (database) => {
  const selectedDatabase = String(database || "").trim();

  // If database is like "abc-2025-26", company settings are usually in main db "abc"
  const mainDatabase = selectedDatabase.replace(/-\d{4}-\d{2}$/, "");

  let companyDetail = await CompanyDetails.findOne({
    database: selectedDatabase,
  });

  // fallback to main/base database
  if (!companyDetail && mainDatabase !== selectedDatabase) {
    companyDetail = await CompanyDetails.findOne({
  ...
  if (!companyDetail) {
    throw new Error(
      `Company details not found for database: ${selectedDatabase}`,
    );
  }

  const lastOrderNo = Number(companyDetail.orderNo) || 0;
  const nextOrderNo = lastOrderNo + 1;

  const orderNO = nextOrderNo.toString().padStart(3, "0");
  const generatedOrderNo = `ORD${orderNO}`;

  companyDetail.orderNo = nextOrderNo;
  await companyDetail.save();
